...
void Kun::addMove(Move* move) {
    ...
    * Add your code here!
    */
	for(vector<Move*>::iterator p = moveList.begin(); p!=moveList.end(); p++){
		if (*p == move) {
			cout<<"This move is already in the list!"<<endl;
			return;
		}
	}
...
Kun& Kun::operator=(const Kun kun) {
    ...
    */
	...
	for (vector<Move*>::const_iterator p=kun.moveList.begin(); p != kun.moveList.end(); p++){
		moveList.push_back(new Move(**p));
	}
	return *this;
}
...
Kun* Kun::operator+(const Kun kun) const {
    ...
    * 
    * create new baby. Need memory allocation through 'new',
    * because the newly born kun need to be added into KunGroup (Container) and it requires dynamic memory.
    ...
    * Add your code here!
    */
	Kun* baby = new Kun(name+kun.name);
	baby->setHP (0.1 * (hp+kun.hp));
	baby->setAttack (0.1 * (attack+kun.attack));
	baby->setDefense (0.1 * (defense+kun.defense));
	for (vector<Move*>::const_iterator p=moveList.begin(); p != moveList.end(); p++){
		baby->moveList.push_back(new Move(**p));
	}
	for (vector<Move*>::const_iterator q=moveList.begin(); q != moveList.end(); q++){
		baby->moveList.push_back(new Move(**q));
	}
	return baby;
}
...
string Kun::getName() const {return name;}

void Kun::setName(string name) {this->name = name;}

int Kun::getLevel() const {return level;}

double Kun::getHP() const {return hp;}

bool Kun::isAlive() const {return (hp > 0);}

void Kun::heal() {hp = level*10;}

double Kun::getAttack() const {return attack;}

void Kun::setAttack(double attack) {this->attack = attack;}

double Kun::getDefense() const {return defense;}

void Kun::setDefense(double defense) {this->defense = defense;}

double Kun::getExperience() const {return experience;}
